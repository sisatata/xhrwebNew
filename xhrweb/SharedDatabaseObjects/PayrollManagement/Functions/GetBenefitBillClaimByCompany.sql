CREATE OR REPLACE FUNCTION payroll.getbenefitbillclaimbycompany(companyid uuid, startdate timestamp without time zone, enddate timestamp without time zone)
 RETURNS TABLE("Id" uuid, "BenefitDeductionId" uuid, "EmployeeId" uuid, "BillDate" timestamp without time zone, "ClaimDate" timestamp without time zone, "AllocatedAmount" numeric, "ClaimAmount" numeric, "Remarks" character varying, "Status" character varying, "ApprovedAmount" numeric, "ApprovedBy" uuid, "ApprovedDate" timestamp without time zone, "ApprovedNotes" character varying, "IsDeleted" boolean, "CompanyId" uuid, "ApprovalStatus" character varying, "LocationFrom" character varying, "LocationTo" character varying, "NameOfAttendees" character varying, "NumberOfAttendees" integer, "VehicleTypeId" uuid,"VehicleTypeName" character varying, "PaidBy" uuid, "PaidDate" timestamp without time zone, "PaymentStatus" integer, "SettledBy" uuid, "SettledDate" timestamp without time zone)
 LANGUAGE plpgsql
AS $function$
	BEGIN
	RETURN QUERY
		SELECT 
		 ...
		 WHERE BenefitBillClaim."IsDeleted" = false and BenefitBillClaim."CompanyId"  = companyId 
		and (startdate is null or BenefitBillClaim."BillDate" >= startdate)
	and (enddate is null or BenefitBillClaim."BillDate" <= enddate);
	END;
$function$
;


DROP FUNCTION getbenefitbillclaimbycompany(uuid,timestamp without time zone,timestamp without time zone) ;