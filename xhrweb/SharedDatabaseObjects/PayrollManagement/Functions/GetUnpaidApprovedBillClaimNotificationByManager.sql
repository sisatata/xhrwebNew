CREATE OR REPLACE FUNCTION payroll.getunpaidapprovedbillclaimnotificationbymanager(managerid uuid, startdate timestamp without time zone, enddate timestamp without time zone)
...
AS $function$
	...
	WHERE bill."PaidBy" = managerid and bill."ApprovalStatus" = '3'
				...
				and (enddate is null or bill."BillDate" ::date <= enddate) or bill."PaymentStatus" = 1))
				order by bill."PaymentStatus", bill."ClaimDate";
	END;
$function$
;

--DROP FUNCTION getunpaidapprovedbillclaimnotificationbymanager(uuid,timestamp without time zone,timestamp without time zone) ;

--select * from payroll.getunpaidapprovedbillclaimnotificationbymanager('59bd9bec-f2ff-4290-90d2-c6d7857fe77e', '2021-01-01', '2021-09-21');