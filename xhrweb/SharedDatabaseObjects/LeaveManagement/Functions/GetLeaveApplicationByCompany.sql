CREATE OR REPLACE FUNCTION leave.getleaveapplicationbycompany(compnayid uuid, startdate timestamp without time zone, enddate timestamp without time zone)
...
inner join main."Company" as c on
	la."CompanyId" = c."Id"
where
	la."CompanyId" = compnayId and ((startdate is null or la."StartDate" >= startdate)
	and (enddate is null or la."StartDate" <= enddate) 
	or (startdate is null or la."EndDate" >= startdate)
	and (enddate is null or la."EndDate" <= enddate)) order by la."StartDate";
...
--DROP FUNCTION getleaveapplicationbycompany(uuid,timestamp without time zone,timestamp without time zone) 
