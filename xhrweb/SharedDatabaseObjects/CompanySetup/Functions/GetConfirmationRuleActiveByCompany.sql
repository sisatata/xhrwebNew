CREATE OR REPLACE FUNCTION main.GetConfirmationRuleActiveByCompany(companyId uuid)
RETURNS TABLE (
		"Id" uuid ,
...
AS $BODY$
	...
	RETURN QUERY
		SELECT 
		 ...
		 WHERE ConfirmationRule."IsDeleted" = False and ConfirmationRule."CompanyId" = companyId 
				and date(ConfirmationRule."StartDate") <= current_date and  date(ConfirmationRule."EndDate") >=current_date;
	END;
$BODY$
LANGUAGE plpgsql;
