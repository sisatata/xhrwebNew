...
{
    public class PrintAttendanceSummaryReportCommandHandler : IRequestHandler<PrintAttendanceSummaryReportCommand, string>
    {
        #region ctor
        public PrintAttendanceSummaryReportCommandHandler(IConfiguration configuration, DbConnection connection, IUriComposer uriComposer)
        {
        ...
        #endregion
        #region methods
        public async Task<string> Handle(PrintAttendanceSummaryReportCommand request, CancellationToken cancellationToken)
        {
            try 
            {
                ...
                var docName = $"Attendance_Summary_Report_{DateTime.Now.ConvertToUnixEpochInSecond()}.pdf";// DateTime.Now.ToString("yyyyMMdd_HHmmss")
                var nn = consolidatedChildrenList;
                var htmlView = await request.Engine.RenderViewToStringAsync("Views/PrintAttendanceSummaryReport.cshtml", consolidatedChildrenList);
